...
const Addons = ({ status, addOns, yearly, updateForm }: updateWithAddons) => {
  ...
  return (
    <motion.div
      ...
      exit="exit"
    >
      <h1 className="mb-2 text-[26px] font-bold text-background ">
        Pick add-ons
      </h1>
      <p className="mb-6 text-lg text-background">
          ...
          >
            <div className="flex items-center">
              ...
              <div className="">
                <Label
                  htmlFor={addon.title}
                  className="block text-base font-medium leading-[.8] text-primary"
                >
                  {addon.title}
                  <span className="mt-2 block text-[12px] text-background">
                    {addon.subtitle}
                  </span>
                </Label>
              </div>
