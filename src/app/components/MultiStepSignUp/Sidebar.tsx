...
const Sidebar = ({ currentStepIndex }: NavTypes) => {
  return (
    <header className="h-[200px] bg-bg-desktop bg-cover bg-center md:h-auto md:rounded-xl md:bg-background md:px-8 ">
        ...
        {["Your Info", "Select Plan", "Add-on", "summary"].map((btn, i) => {
          ...
          return (
            <div className="md:flex md:items-center" key={btn}>
              <button
                    className={`${position} bg-primary flex h-[40px] w-[40px] items-center justify-center rounded-full border border-primary text-default transition-all md:mr-5`}
              >
                {i + 1}
              </button>
